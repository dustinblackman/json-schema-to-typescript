...
function run() {
    ava_1.test('pipe in, pipe out', function (t) {
        t.is(child_process_1.execSync('shx cat ./test/resources/ReferencedType.json | node dist/src/cli.js', { encoding: 'utf-8' }).toString(), expected);
    });
    ava_1.test('pipe in (schema without ID), pipe out', function (t) {
        t.is(child_process_1.execSync('shx cat ./test/resources/ReferencedTypeWithoutID.json | node dist/src/cli.js', { encoding: 'utf-8' }).toString(), expected);
    });
    ...
    ava_1.test('file in (-i), pipe out (absolute path)', function (t) {
        t.is(child_process_1.execSync("node dist/src/cli.js -i " + __dirname + "/../../test/resources/ReferencedType.json").toString(), expected);
    });
